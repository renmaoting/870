...
void Manager::loadSet()
{
  ...
  enemyLeft = static_cast<int>(sprites.size()) -1;
...
bool Manager::checkForCollisions() {
  ...
  while ( sprite != sprites.end()-1 ) {
    if (!(*sprite)->getExplosion() && player->collidedWith(*sprite) )
    {
       sound[3];
       return true;
    }
    int distance =sqrt(pow((*sprite)->X() - player->X(), 2.0) + pow((*sprite)->Y() - player->Y(), 2.0)); 
    if(distance < 300)
    {
        if((*sprite)->velocityX() < 0 && ((*sprite)->X() > player->X()))
        {
...
void Manager::bulletCollision() 
{
    ...
    while(itB != bl.end())
    {
        ...
        {
            if((*itB)->collidedWith(*sprite) && !(*sprite)->getExplosion()) 
            {
                sound[3];
                (*sprite)->explode();
                enemyLeft--; 
...
void Manager::draw() const {
  ...
  viewport.draw();
  hud->draw(bm->getLiveNum(), bm->getFreeNum(), enemyLeft);
  if(gameOver)
      IOManager::getInstance().printMessageCenteredAt("You win! Press 'R' can restart!", 300);
...
void Manager::update() {
  ...
  if(!god && checkForCollisions())// && !player->getExplosion())
  {
    player->explode();
  }
